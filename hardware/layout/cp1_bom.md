...
| C28, C38 | **HV safety Y-cap ~1 nF** (C_stitch) — GND1 pin10 ↔ pin11 (`GND2_DCDC`) across the isolation barrier | 1206 safety (Y1/Y2) | 2 | _verify_ | _verify_ | $0.30 ea (est.) | **MUST be safety-rated to the working voltage, not the 1-min proof: VIORM = 524 Vpk / 396 Vrms continuous per IEC 60747-17** (the 2500 Vrms figure is proof only). Only stitching cap allowed to bridge GND1↔GND2. **No live SKU yet** — pick a rated Murata DE/GA-class Y-cap at CP2 |
