...
| R_inrush | **220 Ω** 1206 **pulse-proof (Vishay CRCW-HP)** 1 %, in the switched branch V24_FUSED→SSR1→R_inrush→V24_SW/C3 | 1206 | 1 | 541-220UCT-ND (2520 stock, Active) | 71-CRCW1206220RFKEAHP (29.5k) | $0.40 | **NEW (F80); re-valued + coordinated (F84).** Limits the recurring turn-on inrush into C3 (22 µF) to **0.134 A @29.2 V — below the SSR's 0.30 A @85 °C *continuous* rating (0.45×)**, so the repetitive event needs no one-shot pulse rating. τ = 220 Ω·22 µF = 4.84 ms; ~24 ms (5τ) to full charge. Per-turn-on I²t = ½CV²/R = **4.3e-5 A²s** < F2 melting I²t 1.9e-4 (survives every turn-on). Under a V24_SW/C3/U2 short it dissipates 0.134²·220 = **3.95 W for ≤5 s** (F2's 200 %→5 s bound) — under the CRCW1206-HP §8.1 short-term-overload guarantee (2.5·√(P·R)/5 s = 9.4 W at P=1.5 W; 4.68 W even at P70=0.75 W). Datasheet Vishay_CRCW_HP.pdf (sha bdd4e4b9). Steady drop ≤3.3 V @15 mA (1.1 V @5 mA typ) |
| F2 | **62 mA** very-fast-acting SMD fuse (Littelfuse 451 Nano2 SMF), switched branch | 1206 | 1 | F3153TR-ND (12.7k stock, Active) | 576-0451.062MRL (6.3k) | $4.57 | **NEW (F84):** coordinated fault protection for the switched display branch. Normal draw ~5 mA (≤~15 mA at e-paper refresh) ≪ 62 mA — no nuisance trip. A downstream short pulls only **0.134 A** (R_inrush-limited) = 216 % of rating → clears within the datasheet's **200 %→5 s max**; the SSR (0.134 A < continuous) and R_inrush (pulse-rated, §8.1) both survive until it clears, so F2 is the *designed* first-fault element (F1 = upstream backstop). 125 V/50 A@125 VDC interrupt; melting I²t 1.9e-4 A²s. Datasheet Littelfuse_451_453.pdf (sha 399d3cc9) |
