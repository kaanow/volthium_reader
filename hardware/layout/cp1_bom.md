...
| J3  | **USB-C receptacle GCT USB4085-GF-A** (native ESP32-S3 USB) | THT top-mount (F13: GCT drawing = "Dip Type, PCB Top Mount"; 20 THT pads) | 1 | 2073-USB4085-GF-ATR-ND (102k stock, Active; API 2026-07-27 — order the cut-tape listing at qty 1) | 640-USB4085-GF-A (84k) | $0.85 | **Δ (D22): was a USB-OTG pin header** — now a board-edge maintenance port (flash/console/JTAG), accessible without opening. ESD-protected by U-ESD. MPN now pinned (was _verify_); matches the schematic's GCT USB4085 footprint and the display J-USB |
...
| J5  | **Würth 61200621621 — keyed 2×3 2.54 mm box header (WR-BHD), the real ESP-Prog "Program" connector: 1=EN, 2=VDD, 3=TXD(target), 4=GND, 5=RXD(target), 6=IO0** | THT | 1 | 732-5394-ND (15,683 stock, Active, API 2026-07-24) | 710-61200621621 (18,083) | $0.72 | **Δ 2026-07-24 (F01): 1×6 → keyed 2×3.** The 1×6 could not mate the ESP-Prog ribbon and had GND/IO0 on the wrong pins. Pinout per ESP-Prog user guide; TXD/RXD are TARGET-perspective — ESP-Prog SCH V2.1 (on file) crosses internally (FT_TXD→0R→ESP_RXD0, FT_RXD→0R→ESP_TXD0), so pin 3 = target TXD (DBG_TXD), pin 5 = target RXD. Ribbon = esptool auto-program; jumper 6→4 + blip pin 1 = manual force-download (DR-32) |
...
| D2 | **DNP** — SOT-23 dual-line TVS footprint only (candidate was onsemi NUP2105L) | SOT-23 | 0 (footprint) | (DNP; candidate NUP2105LT1GOSCT-ND) | | $0 | **Δ 2026-07-24 (F03): populated → DNP, no coordination claimed.** NUP2105L is a 24 V-system CAN protector — onsemi DS (on file): V_BR 26.2–32 V, clamp to 40/44 V @5/8 A — it can NEVER hold the bus below U7's ±14 V abs-max (TCAN332 DS p.5), and no TVS both stands off the ±12 V CAN CM and clamps <14 V. Bus ESD = TCAN332 on-chip (±25 kV HBM / ±12 kV IEC contact, DS p.5). Same intentionally-unprotected call as the RS-485 ports (F44/SM712). Footprint stays as a fit-on-evidence option |
...
**Subtotal ≈ CA$7.6** (U7 3.69 + Q5 1.13 + J6 2.50 + J7 0.20 + R14/R15/C12 0.09; D2 now DNP $0). GPIO note: IO40/41/42 were the last free safe pins — **MCU GPIO budget now exhausted** (JTAG forfeited; debug = J5 UART).
...
| Ref | Part | Pkg | Qty | DigiKey SKU | Mouser SKU | Price | Notes |
|-----|------|-----|-----|-------------|------------|-------|-------|
| J1  | **Würth 615008145521** WR-MJ RJ45 jack, right-angle (horizontal, tab-down), shielded, THT | THT shielded | 1 | 732-615008145521-ND (1.8k stock, Active; API 2026-07-27) | 710-615008145521 (413) | $5.73 | **Δ (DR-10): right-angle** for the shallow box (the earlier SUYIN 100362 is NOT distributor-stocked). **✅ Datasheet-verified 2026-06-25: horizontal/right-angle + shielded (EMI panel finger) + 8P8C + MAGNETICS-FREE** (plain CAT5e jack, 20 mΩ contacts, no magjack) + ~13.6 mm height → fits the ~45 mm depth (~33 mm stack). −40…+85 °C, UL E324776. **Tab-down** → confirm cable-entry orientation at CP3. Datasheet: hardware/datasheets/615008145521.pdf. **Footprint (display CP2): manufacturer-official KiCad footprint vendored into `hardware/kicad/footprints/volthium.pretty/` — the tail fan-out is NON-monotone (x-order 2,1,3,5,4,6,8,7); a hand-derived pattern was rejected. Full verification in `hardware/kicad/footprints/README.md`** |
...
| J2  | **JST-PH 2.0 mm 8-pin** post header (**S8B-PH-K-S, side entry** — the B8B top-entry sibling is rejected, D39) — e-paper SPI: VCC/GND/DIN/CLK/CS/DC/RST/BUSY | THT 1×8 | 1 | S8B-PH-K-S → 455-1725-ND (**stock recovered: 16 → 8,264 by 2026-07-27 (API); the order-early panic is lifted**; Active) | — (Mouser doesn't list; prior 455- cell used the DK prefix) | $0.52 | **Matches the module's PH 2.0 connector — evidence: hashed Waveshare product-page + wiki captures 2026-07-14 (manifest LCD1 row; box lists "PH2.0 20cm 8Pin x1"; iter-23 F21).** Same family both sides → pre-crimped PH↔PH cable (user: ASPHSPH24K102-class), no tool. Keyed by design. **Δ (DR-7):** was a 24-pin FH12-24S FFC (bare-panel) | **Δ CP4 SKU CORRECTION (reviewer F02):** this cell previously held the top-entry sibling's order code, which would have shipped the rejected B8B. Re-resolved live 2026-08-06 (`POST /resolve`) — the code above is the one that returns S8B-PH-K-S. The superseded code is recorded in D39, not here: a wrong order code does not belong in an orderable row. | C6  | 1 µF X7R panel VCC bulk | 0603 | 1 | (same as C5) | | $0.10 | NEW — reduces VCC dip during refresh |
...
| Ref | Part | Pkg | Qty | DigiKey SKU | Mouser SKU | Price | Notes |
|-----|------|-----|-----|-------------|------------|-------|-------|
| BTN1, BTN2, BTN3 | **Same Sky TS02-66-150-BK-160-SCR-D** — 6×6 mm THT tactile, **15.0 mm actuator**, 1.6 ±0.5 N | THT | 3 | 179-TS0266150BK160SC (935 stock; API 2026-08-05) | — | $0.187 ea | **Δ (2026-06-23 user call):** real button protruding through the faceplate (no printed caps). Pick the catalog plunger height nearest (PCB-front→faceplate gap + ~2–3 mm) at CP3/CP5 | **Δ CP4: plunger height RESOLVED.** The CP1 defer ("height from the CP3 stack") is closed: the front-side depth driver is J3's box header at **9.1 mm** (Würth drawing p.1), so the standoff gap is ~9.5 mm and actuator = gap + faceplate 3 + protrusion 2.5 = **15 mm**, which is a catalog height (`150`, not LCR-restricted). Footprint vs datasheet p.2 (reviewer F04): **centres match exactly (6.5 × 4.5 mm); drills deviate 1.0 → 1.1 mm.** The datasheet's recommended pattern is 4 × Ø1.0 mm; the stock KiCad footprint uses Ø1.1 mm. Acceptance: the terminal is 0.7 ± 0.1 mm across its largest dimension (drawing p.2), so a 1.1 mm hole leaves ≥0.3 mm diametral clearance for insertion, and the 2.0 mm pad still gives a 0.45 mm annular ring — comfortably above the 0.13 mm fab floor. This is a sane substitution, **not identity evidence**; if an exact-pattern footprint is wanted, vendor one at CP5. **Push force 160 gf is an engineering pick, not a spec** — 100/260 gf are drop-in siblings if the button feel wants changing; re-confirm the plunger against the CP6 STEP since faceplate thickness is the user's to set.
...
| Ref | Part | Pkg | Qty | DigiKey SKU | Mouser SKU | Price | Notes |
|-----|------|-----|-----|-------------|------------|-------|-------|
| J-USB | **USB-C receptacle GCT USB4115-03-C** — **VERTICAL** (native ESP32-S3 USB, front face) | SMT vertical, 4 THT shell tabs | 1 | 2073-USB4115-03-CTR-ND (54,000 stock, Active; API 2026-08-05) | 640-USB4115-03-C (84k) | $0.85 | **Δ (D27): bench/recovery port** — reached by popping the faceplate (no front cutout); routine updates OTA over RS-485. Replaces the USB-OTG pin header. Same SKU as battery J3 → one footprint + one reel across boards | **Δ CP4 (D40/DR-35): was the edge-mount USB4085.** The edge part cannot be plugged inside the wall box — only ~5 mm clear past any board edge versus a plug needing 15–20 mm, and popping the faceplate exposes the board FRONT, not its edges. The vertical part opens +Z and is reachable once the faceplate and module come away (user call 2026-08-05). H=9.30 mm (drawing p.1) becomes the front-side depth driver over J3's 9.1 mm → gap ~9.7 mm; the 15 mm plunger still gives 2.3 mm protrusion, inside the ~2–3 mm spec, so BTN1–3 are unaffected. 24 contacts of which 16 are used; KiCad stock footprint pad names are a superset so every symbol pin binds. **The battery board's J3 keeps USB4085** — it is not in a wall box and its edge mount is correct there. **Δ CP4 SKU CORRECTION (reviewer F02):** the Mouser cell still held the edge-mount part's order code after D40 moved this to the vertical receptacle; the DigiKey cell was already correct, so one distributor column was updated and the other was not. Re-resolved live 2026-08-06 — the code above returns USB4115-03-C. Superseded code recorded in D40, not here. | U-ESD | USB ESD array (**USBLC6-2SC6Y**) | SOT-23-6 | 1 | 497-11882-2-ND | 511-USBLC6-2SC6Y | $0.30 | **NEW (D27):** ESD clamp on USB-C D+/D−/VBUS. **API-verified 2026-06-25: use `-2SC6Y` (the SC6 is out of stock everywhere)** |
| U3-LDO | 3.3 V LDO (**AP2112K-3.3TRG1**, ~600 mA) | SOT-23-5 | 1 | AP2112K-3.3TRG1DICT-ND (206k stock, Active; API 2026-07-27) | 621-AP2112K-3.3TRG1 (38k) | $0.44 | **NEW (D29):** VBUS→3V3_USB; VBUS-referenced |
| U4-MUX | **TI TPS2116DRLR** priority power mux | **SOT-583 (leadless ⚠)** | 1 | 296-TPS2116DRLRCT-ND (59k stock, Active; API 2026-07-27) | 595-TPS2116DRLR (7.9k) | $1.45 | **NEW (D29):** VIN1=USB-LDO (priority), VIN2=R-78E3.3, OUT=V3V3. No UVLO bypass (display has no U4). **Package SOT-23-6 → SOT-583 (API 2026-06-25)** |
| C_usb1, C_usb2 | LDO in/out 1 µF X7R | 0603 ×2 | 2 | _verify_ | _verify_ | $0.05 ea | **NEW (D29):** LDO caps |
| C_mux | ~47 µF on TPS2116 OUT (V3V3) | 0805/1206 | 1 | _verify_ | _verify_ | $0.10 | **NEW (reviewer F11):** OUT bulk for reverse-current-blocking on USB hot-plug |
| J3  | **Würth 61200621621 keyed 2×3 IDC box header — ESP-Prog "Program"** | THT 2×3 | 1 | 732-5394-ND (15.7k stock, Active; API 2026-07-27) | 710-61200621621 (17.6k) | $0.71 | **Δ display CP2 (DR-32 parity):** this board Deep-sleeps between frames → needs the same USB-independent force-download path as battery J5. Same SKU as battery J5 → one ribbon serves both boards |
| R_cc1, R_cc2 | 5.1 kΩ 0805 — USB-C CC1/CC2 UFP advertisement | 0805 ×2 | 2 | generic | generic | $0.02 ea | **Added at display CP2** (CP1 gap; same class as the battery BOM-diff catch 2026-07-23 — required for a C-to-C cable to enable VBUS) |
