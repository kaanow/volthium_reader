...
| R_inrush | **22 Ω** 1206 1 % (V24_FUSED SSR output → V24_SW/C3) | 1206 | 1 | RMCF1206FT22R0CT-ND | 71-CRCW1206-22-E3 | $0.10 | **NEW (F80):** hard-caps SSR turn-on inrush into C3 (22 µF) to **≤1.33 A @29 V (≤1.09 A @24 V), even at Ron=0 — under the 1.5 A/100 ms one-shot peak.** At the ≤45 mA display load it drops ≤1.1 V (U2 keeps ≥21.9 V ≫ 17 V min); ≤45 mW → 1206 (¼ W). Pulse energy ½·22 µF·29² = 9.3 mJ over <1 ms |
...
one AQY212EH PhotoMOS SSR + R_opto + R_inrush** — net ≈ +$0.8, absorbed in the
...
- Display-feed load switch: **Panasonic AQY212EH PhotoMOS SSR** (Ron 0.85 Ω typ/2.5 Ω max) in series V24_FUSED→**R_inrush 22 Ω**→V24_SW, driven by ESP `PWR_EN` through **R_opto 330 Ω** (~6 mA LED, ≥5 mA worst-case). Replaced the discrete P-FET + gate-driver network (Q1/Q2/R3/Rg/R_base/R_be/DZ1) at iter-48 (F76 resolution) because no discrete driver could be datasheet-guaranteed OFF at temperature. SSR OFF-leakage ≤1 µA @25 °C (spec), rated −40…+85 °C, opto-isolated → cannot self-turn-on. R_inrush caps turn-on inrush ≤1.33 A (<1.5 A one-shot). LED ~20 mW active, 0 in hard-cut.
...
- Q1/Q2: AO3401A/AO3400A (30 V) → **ZXMP6A13F/2N7002** (60 V) — DR-4; iter-38 F57: ZXMP NRND, superseded by **Si2309CDS**
...
- Display-feed switch: discrete P-FET gate network → **AQY212EH PhotoMOS SSR** (iter-48, F76). LED drive **R_opto 330 Ω** (worst-case I_F ≥ 5 mA); series **R_inrush 22 Ω** caps turn-on inrush ≤1.33 A. OFF immunity is architectural (no gate divider), not a resistor value (F79/F80).
