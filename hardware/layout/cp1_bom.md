...
| R_inrush | **150 Ω** 1206 **pulse-proof (Vishay CRCW-HP)** 1 %, in the switched branch V24_FUSED→SSR1→R_inrush→V24_SW/C3 | 1206 | 1 | 541-150UTR-ND (20k stock, Active) | 71-CRCW1206150RFKEAHP (10k) | $0.08 | **NEW (F80); re-valued + coordinated (F84; value re-picked 150 Ω not 220 Ω for cold-temp fuse margin — see below).** Limits the recurring turn-on inrush into C3 (22 µF) to **~0.19 A @29.2 V — below the SSR's 0.30 A @85 °C *continuous* rating (0.63×)**, so the repetitive event needs no one-shot pulse rating. τ = 150 Ω·22 µF = 3.3 ms; ~17 ms (5τ) to full charge. Per-turn-on I²t = ½CV²/R = **6.0e-5 A²s** < F2 melting I²t 1.9e-4 — 3.2× margin (survives every turn-on; melting I²t is adiabatic → holds hot). Under a V24_SW/C3/U2 short it dissipates 0.187²·150 ≈ **5.2 W**; F2 clears fast at ~300 % (≥260 % even at −40 °C — see F2 row) so the exposure is brief, and even at the fuse's worst-case 5 s bound 5.2 W is under the CRCW1206-HP §8.1 short-term-overload guarantee at the part's **1.5 W rating** (2.5·√(P·R)/5 s = 9.4 W/5 s). Datasheet Vishay_CRCW_HP.pdf (sha bdd4e4b9). Steady drop ≤2.3 V @15 mA (0.75 V @5 mA typ) |
| F2 | **62 mA** very-fast-acting SMD fuse (Littelfuse 451 Nano2 SMF), switched branch | 1206 | 1 | F3153TR-ND (12.7k stock, Active) | 576-0451.062MRL (6.3k) | $4.57 | **NEW (F84):** coordinated fault protection for the switched display branch. Normal draw ~5 mA (≤~15 mA at e-paper refresh) ≪ 62 mA — no nuisance trip. A downstream short pulls only **~0.19 A** (R_inrush-limited) = **~300 % of rating** → well above the datasheet's 200 % guaranteed-open threshold, so it clears reliably within the **200 %→5 s max** bound. **Cold-temp margin (why 150 Ω, not 220 Ω): fuses carry more when cold, so at −40 °C (DR-22: sub-zero cabin) the effective % drops ~15 % → still ≈260 % ≫ 200 %.** At 220 Ω the fault was only ~208 % → ~181 % cold, *below* the guaranteed-open threshold — the fuse might not clear and R_inrush would then hold ~4 W continuously (>its 1.5 W). The SSR (~0.19 A < continuous) and R_inrush (pulse-rated, §8.1) both survive to the clear, so F2 is the *designed* first-fault element (F1 = upstream backstop). 125 V/50 A@125 VDC interrupt; melting I²t 1.9e-4 A²s. Datasheet Littelfuse_451_453.pdf (sha 399d3cc9) |
...
one AQY212EH PhotoMOS SSR + R_opto + R_inrush + F2 fuse** — net ≈ **+$6** (added SSR $2.81 + R_opto $0.10 + R_inrush $0.08 + F2 fuse $4.57 ≈ $7.56, less the ~$1.9 discrete gate network removed — the Si2309CDS P-FET alone was ~$1.39; F2 + the SSR dominate), which raises the 2026-07-14
