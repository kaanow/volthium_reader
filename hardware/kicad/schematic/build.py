#!/usr/bin/env python3
"""CP2 schematic — BATTERY-side board entry point (volthium_reader project).

All mechanics (symbol sourcing, geometry, Sheet, and the full gate stack:
readability/glyph/title-block, netlist intent==actual, GOLDEN contracts,
exact-part variants, strict full ERC) live in core.py, shared with the
display-side entry point (build_display.py). This file holds only what is
THIS board's design: the block drawings, the sheet composition, and the
per-board contract tables (GOLDEN / EXACT_PARTS / ERC_ACCEPTED).

Run: <repo>/.venv/bin/python build.py   (from this directory; packet §1)
"""
import sys

import core
from core import Sheet, snap
