...
| TS02-66-150-BK-160-SCR-D | TS02_tactile.pdf (5 pp, dated 11/03/2025) | direct fetch 2026-08-05 from sameskydevices.com (Mouser's mirror WAF-serves HTML to curl — same trap as the GCT drawing at CP3) | https://www.sameskydevices.com/product/resource/ts02.pdf — **display BTN1–3.** p.1 part-number key: `150` = **actuator height 15.0 mm above PCB** (the height derived from the CP4 depth stack), `160` = **1.6 ±0.5 N** push force, `SCR` = short-crimped terminals. 15.0 mm is NOT one of the starred heights restricted to LCR terminals, so this combination is valid. p.1: life 80,000 cycles at 160 gf, −30…+80 °C, 12 Vdc / 50 mA rated (our 3V3 button-to-GND use is far inside). p.2 recommended PCB layout **6.5 × 4.5 mm, 4 × Ø1.0**. Against the stock KiCad `SW_PUSH_6mm_H13mm`: **centres match exactly (6.5 × 4.5 mm); drills deviate 1.0 → 1.1 mm.** The datasheet's recommended pattern is 4 × Ø1.0 mm; the stock KiCad footprint uses Ø1.1 mm. Acceptance: the terminal is 0.7 ± 0.1 mm across its largest dimension (drawing p.2), so a 1.1 mm hole leaves ≥0.3 mm diametral clearance for insertion, and the 2.0 mm pad still gives a 0.45 mm annular ring — comfortably above the 0.13 mm fab floor. This is a sane substitution, **not identity evidence**; if an exact-pattern footprint is wanted, vendor one at CP5. Availability 2026-08-05: 935 in stock, $0.187 CAD, DK 179-TS0266150BK160SC. | fe5026f60ec3 |
| USB4115-03-C | USB4115_drawing.pdf (3 pp, rev B8 17/12/25) | direct fetch 2026-08-05 from gct.co (browser-UA curl; bare curl gets HTML — the CP3 F13 trap) | https://gct.co/files/drawings/usb4115.pdf — **display J-USB, replaces the edge-mount USB4085 per D40/DR-35.** p.1 title block: "USB3.2 Gen2 Type C Receptacle, **Vertical**, SMT, **H=9.30mm**". Opens +Z off the board face, so it is reachable once the faceplate and e-paper module come away, and it overhangs no board edge. 5 A VBUS collectively, 48 V, -25...+85 C, 20,000 mating cycles. 24 signal contacts (USB 3.2) of which our USB 2.0 symbol uses 16; the KiCad stock footprint's pad names are a superset (A1-A12/B1-B12/SH) so every symbol pin binds. Availability 2026-08-05: 54,000 stock, $1.00 CAD, Active. | 8e3448f72b96 |
| S8B-PH-K-S | B8B-PH-K-S.pdf (100K) | digikey | https://www.jst-mfg.com/product/pdf/eng/ePH.pdf — JST **PH family** doc: covers both entry styles. p.3 header table gives bare heights (top-entry B8B 6.0 mm / side-entry S8B 7.6 mm); p.1 gives the top-entry **mated** height 8.0 mm. **CP4/D39 ordered variant = S8B-PH-K-S (side entry)** — the 8.0 mm mated top-entry height consumes the entire PCB→module gap. Filename retains the original download name. | 447624f4f2f7 |
...
| USB4085-GF-A | USB4085-GF-A.pdf (371K) | API proxy 2026-07-27 (display CP2 — battery J3 + display J-USB USB-C pinned to this GCT SKU) | https://gct.co/files/specs/usb4085-spec.pdf — **FAMILY-level product spec** (electrical/mechanical performance; contains no ordering grid, so it does NOT establish the -GF-A object by itself — F13; see the drawing row below for exact-object identity). VBUS = A4/A9/B4/B9 (5 A collective), GND = A1/A12/B1/B12 — matches the USB_C_16P symbol map | e5e6f4f67cca |
| USB4085-GF-A (exact drawing) | USB4085_drawing.pdf (197K, 3 pp) | direct fetch 2026-07-28 (F13 — browser-UA curl; gct.co serves HTML to bare curl) | https://gct.co/files/drawings/usb4085.pdf — GCT official part drawing WITH ordering grid: **"Dip Type, PCB Top Mount" (through-hole — the BOM's earlier "SMD" cell was wrong, F13), suffix decode GF = Gold Flash (Standard), A = Tape & Reel** → USB4085-GF-A fully identified. Matches the KiCad footprint (20 THT pads, 0 SMD) | 39afb82c5104 |
...
| AQY212EH | AQY212EH_Panasonic.pdf (Panasonic GE DIP4 1-Form-A catalog, doc ASCTB126E, 2026.1) | **user upload 2026-07-18 (manufacturer-direct — replaced the octopart mirror; Panasonic host WAF-blocks the proxy)** | Panasonic Industry GE DIP4 1 Form A | — **Display-feed load switch SSR (F76)**: 1-Form-A, **load 60 V / 550 mA**, 5000 Vrms I/O isolation, Ron **0.85 Ω typ / 2.5 Ω max** (AQY212EH row, corrected iter-50 F79 — 0.25 Ω was the AQY211EH sibling), **off-state leakage ≤1 µA @ 25 °C only** (datasheet publishes NO hot max — 85 °C is an engineering estimate gated by a State-4 acceptance test, F83; non-amplifying), LED operate 1.2 mA typ / 3 mA max / turn-off min 0.4 mA, peak load 1.5 A/100 ms one-shot, rated −40…+85 °C. **AQY212EH = through-hole DIP4** (AQY212EHA is the SMD variant). DK 255-2963-ND / Mouser 769-AQY212EH, Active | d329b9729322 |
...
| CRCW120675R0FKEAHP | Vishay_CRCW_HP.pdf (150K, doc 20043 rev 17-Mar-2026) | vishay (API proxy 2026-07-19) | https://www.vishay.com/docs/20043/crcwhpe3.pdf — **Pulse-Proof, High Power** thick-film chip R. p.1: 1206-HP rated **P70 = 0.75 W** (the separate 1.5 W is only at terminal-part temp 105 °C), Umax 200 V. p.8 **§8.1 short-term-overload uses P70**: 2.5·√(P70·R)/5 s → **4.69 W/5 s** guaranteed. p.5 single-pulse curve. **R_inrush = 2× 75 Ω in series (F86): 2.87 W each < 4.69 W** (a single 150 Ω 1206-HP would see 5.74 W). | bdd4e4b9b924 |
| 0451.080MRL | Littelfuse_451_453.pdf (480K, rev 12/01/25) | littelfuse (API proxy 2026-07-19) | https://www.littelfuse.com/assetdocs/littelfuse-fuse-451-453-datasheet — 451/453 Nano2® SMF **Very-Fast-Acting** fuse. p.1: 200%→**5 s max** opening. p.2 (**.080 row**): **125 V**, **50 A @125 VDC** interrupt, Rcold 4.05 Ω, **nominal melting I²t 3.3e-4 A²s**. **F2 switched-branch fault clear (80 mA, F87 — 62 mA's pulse I²t ratio was too high).** | 399d3cc9da99 |
...
| TCAN332DR | TCAN332.pdf (1645K, SLLSEQ7F Rev May 2025) | parts-API /datasheet proxy 2026-07-22 | https://www.ti.com/lit/ds/symlink/tcan332.pdf | aeda66d5fca9 | U7 Xanbus CAN transceiver (DR-31); pinout + supply-current + unpowered-high-Z interface-verified (pp.1/4/6); **p.5 abs-max V(BUS) ±14 V + on-chip bus ESD ±25 kV HBM / ±12 kV IEC contact (F03 basis)** |
| NUP2105LT1G (onsemi) | NUP2105L_onsemi.pdf (13 pp, Rev 12 June 2026) | parts-API /datasheet proxy 2026-07-24 | https://www.onsemi.com/pdf/datasheet/nup2105l-d.pdf | bf93369469d4 | D2 candidate CAN TVS, **now DNP (CP2 F03)**: p.2 V_RWM 24 V, V_BR 26.2–32 V, V_C 40/44 V max @5/8 A — cannot bracket TCAN332's ±14 V; kept on file as the evidence for the DNP call. (Note: the bare "NUP2105L" MPN resolves to a Diotec second-source PDF at DigiKey — query the orderable NUP2105LT1G for the onsemi doc) |
| ESP-Prog (tool, not BOM) | ESP-Prog_SCH_V2.1.pdf (609K, SCH_ESP32-PROG_V2.1_20190709) | espressif direct 2026-07-24 | https://dl.espressif.com/dl/schematics/SCH_ESP32-PROG_V2.1_20190709.pdf | ab1f074c7b6c | J5 Program-pinout ground truth (F01): J3/J5 2×3 = 1 ESP_EN / 2 VDD / 3 ESP_TXD0 / 4 GND / 5 ESP_RXD0 / 6 ESP_IO0; **FT_TXD→0R→ESP_RXD0, FT_RXD→0R→ESP_TXD0** (names are target-perspective — pin 3 takes the target's TXD) |
| MPZ2012S601AT000 | TDK_MPZ2012S601AT000.pdf (MPZ2012 type catalog, 20190424) | parts-API /datasheet proxy 2026-07-24 | https://product.tdk.com/system/files/dam/doc/product/emc/emc/beads/catalog/beads_commercial_power_mpz2012_en.pdf | c8b428baaacf | L10–L13 iso beads. p.1 table: S601A = 600 Ω ±25 % @100 MHz, DCR 0.100 Ω max, **rated 2 A** (derates above 85 °C). p.2 Z/f curve (S601A): **~600 Ω near the 150–200 MHz peak, falling to ~300–400 Ω by ~400 MHz** — DR-30 bead-impedance data: no MPZ2012 reaches the ADM2587E p.17 "~2 kΩ" guidance (family max = S102A 1 kΩ/1.5 A/0.15 Ω, an available same-size step-up at BOM-lock) |
| C1206C102KDGACTU | KEMET_C1206C102KDGACTU.pdf (KEM_C1009 C0G HV SMD, 12/16/2024) | parts-API /datasheet proxy 2026-07-24 | https://yageogroup.com/content/datasheet/asset/file/KEM_C1009_C0G_HV_SMD | 46031e2efdb6 | C28/C38 C_stitch. High-Voltage C0G series doc: 500 V–10 kV DC ratings incl. **1 kV 1206**, −55…+125 °C, ±30 ppm/°C — the 1 kV/C0G claims in the BOM row are this doc's |
| 61200621621 | Wurth_61200621621.pdf (WR-BHD male box header drawing, rev 002.001 2026-07-07) | parts-API /datasheet proxy 2026-07-24 | https://www.we-online.com/components/products/datasheet/61200621621.pdf | 6474a7ec48fc | J5 ESP-Prog Program header (F01): 6-pin 2.54 mm shrouded/keyed box header, **standard IDC odd/even numbering (1/3/5 one row, 2/4/6 other — matches Conn_02x03_Odd_Even + the ESP-Prog ribbon 1:1)**, THT, hole pattern p.1. **CP3 note: set key-notch orientation at placement so ribbon pin 1 = our pin 1** |

## Retired (in store history, not used)

- SI2309CDS_Vishay.pdf (feb0974dd371), MMBT5551_onsemi.pdf (3ae47cf6f802),
  BZX84C12LT1G.pdf (f9818d9dfc03) — retired iter-48 (F76): the discrete
  Q1 P-FET + Q2 BJT gate-driver + Zener were replaced wholesale by the
  AQY212EH PhotoMOS SSR (no discrete driver could be datasheet-guaranteed
  OFF at temperature — see decisions.md D19). PDFs kept as F57/F68/F72
  evidence; can be removed at the next housekeeping pass.
