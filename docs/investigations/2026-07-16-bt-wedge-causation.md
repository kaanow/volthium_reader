...
| 2026-07-18T07:22:47Z→ongoing (**battery A BLE dormancy ≥2 h 24 min, DURING DISCHARGE**; crossed 15-min alert; NOT a wedge) | n/a — ambient OFF. **B read cleanly on hci0 the entire time** (SOC 64%→56%, discharging) while **A stayed fully null** from 07:23:12Z through ≥09:47Z. Live BlueZ probe at 09:47Z: **A absent from the bluez object list entirely** (not connected, not cached), B present at RSSI -76, hci0 UP with 0 errors, zero active LE connections | **Family A2, but now a categorically longer outage (≥2.4 h vs prior max 23 min) — and confirmed peer-side by direct probe.** A not being in the bluez object list *rules out* a leaked/stale connection (FM-8 would show `Connected: yes`); A is simply not advertising where hci0 can hear it. **Electrical safety confirmed by topology:** series pack → identical current through both; B is actively discharging, which is only possible if current flows through A, so A's discharge FET is closed and A is conducting normally. **A's BMS works electrically; only its BLE radio is dormant.** Battery not in danger; the fuller cell (A was ~100%) is just invisible | **Reader correct**: partial rows preserve B, no false wedge, no recovery churn. Deliberately did NOT run a manual scan on hci0 to probe A (would race the reader's own discovery → the InProgress stressor). **Two real takeaways:** (1) **blind spot** — a battery we can't see over BLE can't trip our cloud undervoltage alert; under load that's a genuine monitoring gap. (2) **This is the strongest argument yet for the wired RS-485/CAN path** (see docs/vendor/): a BMS whose BLE stack hangs for hours is exactly the failure a wired serial/CAN link doesn't have. BLE was always the fragile leg. Also surfaced: **A/B SOC imbalance** (A ~100% vs B ~56% at dormancy) worth its own look |
