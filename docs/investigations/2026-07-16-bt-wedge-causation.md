...
| 2026-07-24T05:31→12:54Z (**persistent-connection dual-adapter experiment — ENDED by an unrecovered InProgress wedge**) | Ran the A/B experiment (commit e2886fd): B held persistent on the internal BCM43438, A on/off on the UB500. **~7 h of clean dual-adapter operation** — B on a SINGLE `persist_connect` with thousands of `persist_read_ok`, **zero drops**, cadence nearly doubled (~1150 rows/2 h), zero wedges. Then at **12:54:31Z a `scan_error [org.bluez.Error.InProgress]`** on the primary, B's held link timed out 17 s later (12:54:48), repeated InProgress + a wedge_snapshot, and the logger **stuck ~28 min** (read #4245, "25 in a row", both batteries blind) — the recovery ladder did NOT self-clear it. Ended the experiment (removed the `20-persist-experiment.conf` drop-in) + restarted → single-adapter restored, both reading immediately. | **Two separable conclusions.** (1) **The persistent held connection is VIABLE** — 7 h on one connect, no idle-timeout, no drops answers the "untested BMS behaviour under persistence" worry: this BMS holds a link fine. (2) **Dual-adapter is NOT viable on this Pi as-built** — it reintroduced the FM-3 InProgress wedge (the exact failure the single-adapter experiment was created to escape), and worse, the recovery ladder failed to clear it in the persistent flow → an *unattended-unsafe* unrecovered outage. The trigger order (scan_error at 12:54:31 *before* B's drop at 12:54:48) is ambiguous — could be a spontaneous primary-discovery InProgress that cascaded, or B's reconnect scan colliding with A's discovery; 4 events isn't enough to call it. | **Decisive-negative on dual-adapter; INCONCLUSIVE on the churn hypothesis** — no dormancy occurred on either battery in the 7 h (0 `persist_absent`, no silent stretches), so NO B-vs-A comparison data was collected before the wedge cut it short. Code left in place (flag-gated, now OFF — harmless dormant). **Paths forward if we retry:** (a) fix the recovery ladder to clear InProgress in the persistent/dual-adapter flow first; (b) get persistence WITHOUT dual-adapter — hold B + cycle A on the SAME hci0 (multi-connection), sidestepping concurrent-discovery InProgress entirely; or (c) the cheaper single-adapter slow-poll churn test. The persistent MECHANISM is proven; the delivery vehicle (dual-adapter) is what failed. |
