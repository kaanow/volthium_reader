...
| 2026-07-18T07:22:47Z→ongoing (**battery A dormancy ≥24 min, DURING DISCHARGE**; crossed 15-min alert; NOT a wedge) | n/a — ambient OFF. **B read cleanly on hci0 throughout** (SOC 64%, v_b 13.08 V falling, state=discharging) while **A went fully null** at 07:23:12Z and had not returned as of 07:47:24Z. hci0 provably healthy (B fine on it) → peer-specific to A | **Family A2 again, but two firsts: longest dormancy yet (≥24 min vs prior max 23) and the first to occur under load rather than at idle.** A's BMS stopped advertising while the pack was actively discharging — so we're monitoring only half the pack under current. Dormancy durations are trending up across events (A 3.5 min → B 13 min → B 23 min → A ≥24 min). Still not chip, not our stack (2nd A dormancy; both batteries now do it) | **Reader correct**: partial rows preserve B, no false wedge, no recovery churn (a peer that isn't advertising can't be fixed on hci0). Alert fired appropriately at 15 min. **Operational gap surfaced**: a single-battery dormancy under discharge leaves that cell's undervoltage protection unobservable to us — the cloud undervoltage alert can't fire on a battery we can't see. Worth considering: (1) a longer-dormancy escalation (e.g. at N min, attempt a directed reconnect / L1 in case it's a latent FM-8 on A), (2) surfacing "monitoring degraded under load" distinctly from idle dormancy. Low code urgency but a real blind spot |
