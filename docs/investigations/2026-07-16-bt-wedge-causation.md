...
| 2026-07-18T07:22:47Z→ongoing (**battery A BLE dormancy ≥4 h 24 min, DURING DISCHARGE**; crossed 15-min alert; NOT a wedge) | n/a — ambient OFF. **B read cleanly on hci0 the entire time** (SOC 64%→49%, discharging, v_b 13.06 V) while **A stayed fully null** from 07:23:12Z through ≥11:47Z. Live BlueZ probe at 09:47Z: **A absent from the bluez object list entirely** (not connected, not cached), B present at RSSI -76, hci0 UP with 0 errors, zero active LE connections | **Family A2, confirmed peer-side by direct probe.** NOT categorically new — precedent is the 2026-07-17 **B dormancy of ~4 h 48 min** (08:32→13:20Z); multi-hour BMS-BLE dormancies are an established pattern, so A returning near the ~5 h mark would fit. A absent from bluez *rules out* a leaked/stale connection (FM-8 shows `Connected: yes`); A simply isn't advertising where hci0 can hear it. **Electrical safety from topology:** series pack → identical current; B actively discharging ⇒ current flows through A ⇒ A's discharge FET closed ⇒ A conducting normally. **A's BMS works electrically; only its BLE radio is dormant.** Battery safe; the fuller cell (A ~100%) is just invisible | **Reader correct**: partial rows preserve B, no false wedge, no recovery churn. Deliberately did NOT run a manual scan on hci0 (would race the reader's discovery → InProgress stressor). **Takeaways:** (1) **blind spot** — an unseen battery can't trip our undervoltage alert; a real monitoring gap under load. (2) **Strongest argument yet for the wired RS-485/CAN path** (docs/vendor/): a BMS whose BLE hangs for hours is exactly what a wired link doesn't suffer. (3) **Candidate mitigation to prototype:** attempt a *directed* connect to A's MAC on dormancy (some BMSes drop advertising but still accept directed connections) — but only via burst-adapter (hci1) to avoid racing hci0. (4) Watch **A/B SOC imbalance** (A ~100% vs B now 49%) separately |
